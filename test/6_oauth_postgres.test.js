...
describe('POSTGRES OAUTH2', async () =>
{
    before(async () =>
    {
    ...
    {
        const { data:[{ email, username }] } = await axios.post(`/users`, users.jose)
        assert.equal(email, users.jose.email)
        assert.equal(username, users.jose.username)
    })

    it(`should not be able to create duplicate user`, async () =>
    {
        const { data } = await axios.post(`/users`, users.jose)
        assert.equal(data.constraint, `users_email_key`)
    })

    it(`user should authenticate pg`, async () =>
    {
